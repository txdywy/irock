...
irock_hy2_result irock_hy2_session_open_tcp_stream(irock_hy2_session_ref session, const char *address, irock_hy2_stream_ref *stream) {
  ...
  struct irock_hy2_stream *created_stream = irock_hy2_stream_create(hy2_session, stream_id, request_buffer, request_bytes_written);
  if (!created_stream) {
    return IROCK_HY2_NETWORK_FAILED;
  }
...
irock_hy2_result irock_hy2_session_open_raw_quic_stream(irock_hy2_session_ref session, int bidirectional, const uint8_t *initial_payload, int initial_payload_length, irock_hy2_stream_ref *stream) {
  ...
  if (!session || !stream || initial_payload_length < 0 || (initial_payload_length > 0 && !initial_payload)) {
    return IROCK_HY2_INVALID_CONFIGURATION;
  }
  struct irock_hy2_session *hy2_session = session;
  if (!hy2_session->authenticated) {
  ...
  int64_t stream_id = -1;
  int open_result = bidirectional ? ngtcp2_conn_open_bidi_stream(hy2_session->quic_conn, &stream_id, 0) : ngtcp2_conn_open_uni_stream(hy2_session->quic_conn, &stream_id, 0);
  if (open_result == NGTCP2_ERR_INVALID_STATE || open_result == NGTCP2_ERR_STREAM_ID_BLOCKED) {
    return IROCK_HY2_BLOCKED;
  }
  if (open_result != 0) {
  ...
  struct irock_hy2_stream *created_stream = irock_hy2_stream_create(hy2_session, stream_id, initial_payload, initial_payload_length);
  if (!created_stream) {
    return IROCK_HY2_NETWORK_FAILED;
  }
  irock_hy2_result register_result = irock_hy2_session_register_stream(hy2_session, created_stream);
...
irock_hy2_result irock_hy2_session_create_raw_quic_stream_for_testing(irock_hy2_session_ref session, int64_t stream_id, const uint8_t *initial_payload, int initial_payload_length, irock_hy2_stream_ref *stream) {
  ...
  if (!session || stream_id < 0 || !stream || initial_payload_length < 0 || (initial_payload_length > 0 && !initial_payload)) {
    return IROCK_HY2_INVALID_CONFIGURATION;
  }
...
irock_hy2_result irock_hy2_session_create_tcp_stream_for_testing(irock_hy2_session_ref session, int64_t stream_id, irock_hy2_stream_ref *stream) {
  return irock_hy2_session_create_raw_quic_stream_for_testing(session, stream_id, 0, 0, stream);
}
