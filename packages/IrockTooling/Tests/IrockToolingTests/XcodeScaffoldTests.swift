...
final class XcodeScaffoldTests: XCTestCase {
    ...
    func testMacOSAppWiresLocalProxyControllerForUnsignedUsability() throws {
        ...
        let udpClient = try String(contentsOf: repositoryRoot.appendingPathComponent("apps/irock-macOS/irockMacTunnelExtension/MacOSPlatformUDPDatagramClient.swift"))
        ...
        XCTAssertTrue(project.contains("MacOSPlatformTCPDialer.swift in Sources"))
        XCTAssertTrue(project.contains("MacOSPlatformUDPDatagramClient.swift in Sources"))
        ...
        XCTAssertTrue(controller.contains("NativeTUICQUICSessionDialer"))
        XCTAssertTrue(controller.contains("TUICStreamOpener"))
        XCTAssertTrue(controller.contains("openTUICOutboundAndRelay"))
        XCTAssertTrue(controller.contains("connectQUICSession"))
        XCTAssertTrue(controller.contains("exportKeyingMaterial"))
        XCTAssertTrue(controller.contains("case .tuic:\n            return node.transport == .quic\n                && node.tls.enabled"))
        XCTAssertFalse(project.contains("MacOSPlatformQUICStreamDialer.swift in Sources"))
        ...
        XCTAssertTrue(controller.contains("openTrustTunnelOutboundAndRelay"))
        XCTAssertTrue(controller.contains("HTTP2ClientByteStream"))
        XCTAssertTrue(controller.contains("method: \"CONNECT\""))
        XCTAssertTrue(controller.contains("proxy-authorization"))
        ...
        XCTAssertTrue(tunController.contains("TunnelRuntimeController.makeShadowsocksTCPSession"))
        XCTAssertTrue(tunController.contains("TunnelRuntimeController.makeVMessGRPCSession"))
        XCTAssertTrue(tunController.contains("stream: MacOSPlatformTCPByteStreamDialer()"))
        XCTAssertTrue(tunController.contains("TunnelRuntimeController.makeTUICQUICSession"))
        XCTAssertTrue(tunController.contains("TunnelRuntimeController.makeTrustTunnelHTTP2Session"))
        XCTAssertTrue(tunController.contains("case .trustTunnel where node.transport == .http2"))
        XCTAssertTrue(tunController.contains("runtime.runOnce()"))
        XCTAssertTrue(tunController.contains("runtime.closeProxyConnections()"))
        XCTAssertTrue(tunController.contains("NativeTUICQUICSessionDialer"))
        XCTAssertTrue(tunController.contains("ProtocolUDPDatagramForwarder(client: MacOSPlatformUDPDatagramClient(), proxyAdapterRegistry: registry)"))
        XCTAssertTrue(udpClient.contains("final class MacOSPlatformUDPDatagramClient: UDPDatagramClient"))
        XCTAssertTrue(udpClient.contains("NWConnection(host: endpointHost, port: endpointPort, using: .udp)"))
        XCTAssertTrue(udpClient.contains("connection.send(content: Data(request.payload)"))
        XCTAssertTrue(udpClient.contains("connection.receiveMessage"))
        ...
        XCTAssertTrue(tunCredentialResolver.contains("struct MacOSImportedProxyCredentialResolver: ProxyCredentialResolver"))
        XCTAssertTrue(tunCredentialResolver.contains("typealias MacOSImportedShadowsocksCredentialResolver = MacOSImportedProxyCredentialResolver"))
    ...
    func testMacOSPacketTunnelRuntimeIntegrationUsesSharedRuntimeAndPlatformTCPDialer() throws {
        ...
        XCTAssertTrue(smokeRunner.contains("TunnelRuntimeController.makeShadowsocksTCPSession"))
        XCTAssertTrue(smokeRunner.contains("TunnelRuntimeController.makeVMessGRPCSession"))
        XCTAssertTrue(smokeRunner.contains("stream: MacOSPlatformTCPByteStreamDialer()"))
        XCTAssertTrue(smokeRunner.contains("TunnelRuntimeController.makeTUICQUICSession"))
        XCTAssertTrue(smokeRunner.contains("TunnelRuntimeController.makeTrustTunnelHTTP2Session"))
        XCTAssertTrue(smokeRunner.contains("case .trustTunnel where snapshot.selectedNode.transport == .http2"))
        XCTAssertTrue(smokeRunner.contains("NativeTUICQUICSessionDialer"))
        XCTAssertTrue(smokeRunner.contains("ProtocolUDPDatagramForwarder(client: MacOSPlatformUDPDatagramClient(), proxyAdapterRegistry: registry)"))
        XCTAssertTrue(smokeRunner.contains("KeychainProxyCredentialResolver"))
        XCTAssertTrue(smokeRunner.contains("SecItemCopyMatching"))
        ...
        XCTAssertTrue(project.contains("IROCKTUNNEL000000000000001 /* irockMacTunnelExtension */"))
        XCTAssertTrue(project.contains("IROCKPKGNATIVEHY20000001 /* IrockNativeHysteria2 */"))
        XCTAssertTrue(project.contains("MacOSPlatformUDPDatagramClient.swift in Sources"))
    ...
    func testContainerAppDeclaresAppGroupRuntimeStoreResolver() throws {
        ...
        let extensionResolver = try String(contentsOf: repositoryRoot.appendingPathComponent("packages/IrockTunnelCore/Sources/IrockTunnelCore/PacketTunnelAppGroupStoreResolver.swift"))
        let project = try String(contentsOf: repositoryRoot.appendingPathComponent("apps/irock-iOS/irock.xcodeproj/project.pbxproj"))
    ...
    func testNEPacketTunnelFlowAdapterWritesOnlyExplicitResponseBytes() throws {
        let flowAdapter = try String(contentsOf: repositoryRoot.appendingPathComponent("packages/IrockTunnelCore/Sources/IrockTunnelCore/NEPacketTunnelFlowPacketFlowIO.swift"))
    ...
    func testPacketTunnelSmokePathFilesDeclareExpectedBoundaries() throws {
        let flowAdapter = try String(contentsOf: repositoryRoot.appendingPathComponent("packages/IrockTunnelCore/Sources/IrockTunnelCore/NEPacketTunnelFlowPacketFlowIO.swift"))
        let storeResolver = try String(contentsOf: repositoryRoot.appendingPathComponent("packages/IrockTunnelCore/Sources/IrockTunnelCore/PacketTunnelAppGroupStoreResolver.swift"))
        let smokeRunner = try String(contentsOf: repositoryRoot.appendingPathComponent("apps/irock-iOS/irockTunnelExtension/IOSPacketTunnelSmokeRunner.swift"))
    ...
    func testPlatformImportsStayInsidePlatformTargets() throws {
        let forbiddenPackageModules = Set(["UIKit", "AppKit"])
    ...
    func testPacketTunnelRuntimeSettingsFilesDeclareExpectedDefaults() throws {
        let configuration = try String(contentsOf: repositoryRoot.appendingPathComponent("packages/IrockTunnelCore/Sources/IrockTunnelCore/PacketTunnelRuntimeSettingsConfiguration.swift"))
        let factory = try String(contentsOf: repositoryRoot.appendingPathComponent("packages/IrockTunnelCore/Sources/IrockTunnelCore/PacketTunnelRuntimeSettingsFactory.swift"))
        let applicator = try String(contentsOf: repositoryRoot.appendingPathComponent("packages/IrockTunnelCore/Sources/IrockTunnelCore/PacketTunnelRuntimeSettingsApplicator.swift"))
